...
DynThresholdParamN Conversion::DynThresholdParamToNative(Yoga::Wrapper::DynThresholdParam ^ param)
{
	DynThresholdParamN paramN;

	String^ strNew = param->lightDark; //cli的string.  

	//cli的string转std::string  
	paramN.lightDark = marshal_as<std::string>(strNew);
	paramN.offset = param->offset;
	paramN.maskWidth = param->maskWidth;
	paramN.maskHeight = param->maskHeight;
	paramN.closingRadius = param->closingRadius;
	paramN.openingRadius = param->openingRadius;
	paramN.leastArea = param->leastArea;
	return paramN;
}

OcrParamN Conversion::OcrPramToNative(Yoga::Wrapper::OcrParam ^ param)
{
	OcrParamN paramN;
	String^ polarity = param->polarity; //cli的string. 
	String^ ocrType = param->ocrType; //cli的string.
	String^ expression = param->expression; //cli的string. 

	paramN.polarity= marshal_as<std::string>(polarity);
	paramN.ocrType = marshal_as<std::string>(ocrType);
	paramN.expression = marshal_as<std::string>(expression);

	paramN.thresholdValue = param->thresholdValue;
	paramN.closeCircleRadius = param->closeCircleRadius;
	paramN.openCircleRadius = param->openCircleRadius;
	paramN.minCharArea = param->minCharArea;
	paramN.maxCharArea = param->maxCharArea;
	paramN.minCharWidth = param->minCharWidth;
	paramN.maxCharWidth = param->maxCharWidth;
	paramN.minCharHeight = param->minCharHeight;
	paramN.maxCharHeight = param->maxCharHeight;
	paramN.partitionCharWidth = param->partitionCharWidth;
	return paramN;
}
