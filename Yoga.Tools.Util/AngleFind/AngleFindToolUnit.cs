...
{
    public partial class AngleFindToolUnit : ToolsSettingUnit
    {
        private AngleFindTool tool;
        /// <summary>halcon窗体控制</summary>
        public HWndCtrl mView;
        /// <summary>ROI 管理控制</summary>
		public ROIController ROIController;
        public AngleFindToolUnit(AngleFindTool tool)
        {
            InitializeComponent();
        ...
        private void UpdateViewData(object sender, ViewEventArgs e)
        {
            switch (e.ViewMessage)
            {
                ...
                case ViewMessage.CreatedROI:
                    ...
                    ROI circleRoi = ROIController.ROIList.Find(x=>x is ROICircle);
                    if (circleRoi != null)
                    {
                        tool.CircleRegion = circleRoi.GetRegion();                       
                    }
                    else
                    {
                        tool.CircleRegion = null;
                    }
                    ROI angleRoi = ROIController.ROIList.Find(x=>x is ROIRectangle2);
                    if (angleRoi!=null)
                    {
                        tool.AngleRegion = angleRoi.GetRegion();
                    }
                    else
                    {
                        tool.AngleRegion = null;
                    }
        ...
        {
            ROIController.SetROIShapeNoOperator(new ROIRectangle2());
        }
        ...
        private void TestModelButton_Click(object sender, EventArgs e)
        {
            ...
            string resultMessage = string.Format("查找结果\r\n编码格式: {0}\r\n找到字符{1}\r\n结果字符串: {2}\r\n用时:{3:f2}",
                0, 0, tool.Result,tool.ExecutionTime);
            ShowModelGraphics();
        ...
        private void btnSetCircleROI_Click(object sender, EventArgs e)
        {
            int index = ROIController.ROIList.FindIndex(x => x is ROICircle);
            if (index>-1)
            {
                ROIController.RemoveROI(index);
            }
            ROIController.SetROIShape(new ROICircle());
        }

        private void btnSetAngleROI_Click(object sender, EventArgs e)
        {
            int index = ROIController.ROIList.FindIndex(x => x is ROIRectangle2);
            if (index > -1)
            {
                ROIController.RemoveROI(index);
            }
